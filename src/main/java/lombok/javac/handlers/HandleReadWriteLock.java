...
public class HandleReadWriteLock {
    ...
    private static void handleReadWriteLock(
        ...
        if (lockFieldName == null || lockFieldName.isEmpty()) {
            lockFieldName =
                    annotationNode.getAst().readConfiguration(ConfigurationKeys.READ_WRITE_LOCK_DEFAULT_FIELD_NAME);
            if (lockFieldName == null || lockFieldName.isEmpty()) {
        ...
        if (fieldExists(lockFieldName, methodNode) == MemberExistsResult.NOT_EXISTS) {
            ...
            JCVariableDecl fieldDecl = JavacHandlerUtil.recursiveSetGeneratedBy(maker.VarDef(
        ...
        JCTree.JCBlock finalizer =
                JavacHandlerUtil.recursiveSetGeneratedBy(maker.Block(0, readLockUnlockStatements), methodNode);
        method.body = JavacHandlerUtil.setGeneratedBy(maker.Block(
                0,
                List.of(
                        readLockVariable,
                        readLockLockStatement,
                        JavacHandlerUtil.setGeneratedBy(maker.Try(method.body, List.nil(), finalizer), methodNode)
                )
        ), methodNode);
